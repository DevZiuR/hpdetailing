  ...
  const [messages, setMessages] = useState<ChatMessage[]>(() => [
    {
      ...
      role: "assistant",
      content:
        "Welcome to High Performance Detailing. I can help you pick the right package, check availability, or answer questions about our services.",
      timestamp: Date.now(),
    },
  ]);
          ...
          <div className={`relative flex items-center justify-between px-4 py-3 bg-[hsl(var(--card))] border-b border-border/60 ${headerGlow}`}>
            <div className="flex items-center gap-2">
              ...
              <div className="leading-tight">
                ...
                <div className="text-[11px] text-muted-foreground">High Performance Detailing</div>
              </div>
            </div>
            <button
...
function getAssistantReply(userText: string) {
  ...
  if (/(price|cost)/.test(t)) {
    return "Our detailing packages start with an Exterior Refresh and scale up to a Signature Ceramic Coat. Tell me your vehicle type and what you’re looking for, and I’ll recommend a package.";
  }
  if (/(book|availability|schedule|appointment)/.test(t)) {
    return "We offer flexible scheduling. Share your preferred date and time window, and I’ll check availability for you.";
  }
  if (/(ceramic|coating)/.test(t)) {
    return "Our ceramic coatings provide long-lasting protection and a deep gloss. We offer 1–5 year options depending on your goals.";
  }
  if (/(location|where)/.test(t)) {
    return "We’re located in the Miami area. If you share your neighborhood, I can confirm service options and travel arrangements.";
  }
  return "Got it. Would you like a personalized recommendation based on your vehicle and goals?";
}
